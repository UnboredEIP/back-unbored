...
describe('AuthController', () => {
    ...
    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            imports: [AuthModule, DatabaseModule.forRoot("mongodb://localhost:27017/unboredAuthEnv"), 
    ...
    describe('login', () => {
        ...
        it ('should login an account', async() => {
            const response = await request(httpServer).post('/auth/login').send(User1)
    ...
    describe('refresh token', () => {
        it ('should return me new token', async() => {
            await request(httpServer).post('/auth/register').send(User1);
            const myTokens = await request(httpServer).post('/auth/login').send(User1);
