...
describe('EventController', () => {
    ...
    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            imports: [GroupModule, AuthModule, DatabaseModule.forRoot("mongodb://localhost:27017/unboredGroupEnv"), 
    ...
    beforeEach(async () => {
        ...
        groupUserBearer = (await request(httpServer).post('/auth/login').send(User1)).body.token
        groupUser2Bearer = (await request(httpServer).post('/auth/login').send(User2)).body.token
    ...
    describe('user groups', () => {
        ...
        it("should create a group", async() => {
            const response = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            expect(response.status).toBe(HttpStatus.CREATED)
        });

        it ("should not create a group (conflit)", async() => {
            await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            const response = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
    ...
    describe('show groups', () => {
        it("should return groups informations", async() => {
            const group = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            const groupId = group.body.group._id
            const response = await request(httpServer).get('/group/show?group_id='+groupId).set('Authorization', 'Bearer ' + groupUserBearer)
        ...
        it("should not return groups informations (bad id)", async() => {
            ...
            const response = await request(httpServer).get('/group/show?group_id='+badGroupId).set('Authorization', 'Bearer ' + groupUserBearer)
    ...
    describe('invitation in groups', () => {
        it("should invite user 2 in groups", async() => {
            ...
            const group = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            const groupId = group.body.group._id;
            const response = await request(httpServer).post('/group/invite?group_id='+groupId+'&'+'user_id='+user2Id).set('Authorization', 'Bearer ' + groupUserBearer)
            expect(response.body.status).toBe(HttpStatus.OK)
        ...
        it("should not invite user 2 in groups (invitation duplication) ", async() => {
            ...
            const group = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            const groupId = group.body.group._id;
            await request(httpServer).post('/group/invite?group_id='+groupId+'&'+'user_id='+user2Id).set('Authorization', 'Bearer ' + groupUserBearer)
            const response = await request(httpServer).post('/group/invite?group_id='+groupId+'&'+'user_id='+user2Id).set('Authorization', 'Bearer ' + groupUserBearer)
        ...
        it("should not invite user 2 in groups (bad group id)", async() => {
            const user2Id = (await dbConnection.collection('users').findOne({username: User2.username}))._id
            const badGroupId = new Types.ObjectId();
            const response = await request(httpServer).post('/group/invite?group_id='+badGroupId._id+'&'+'user_id='+user2Id).set('Authorization', 'Bearer ' + groupUserBearer)
            expect(response.body.statusCode).toBe(HttpStatus.NOT_FOUND)
        ...
        it("should not invite user 2 in groups (bad user id) ", async() => {
            ...
            const group = await request(httpServer).post('/group/create').set('Authorization', 'Bearer ' + groupUserBearer).send({name: "heelo"})
            const groupId = group.body.group._id;
            const response = await request(httpServer).post('/group/invite?group_id='+groupId+'&'+'user_id='+badUserId._id).set('Authorization', 'Bearer ' + groupUserBearer)
            expect(response.body.statusCode).toBe(HttpStatus.NOT_FOUND)
