...
describe('EventController', () => {
    ...
    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            imports: [EventModule, AuthModule, DatabaseModule.forRoot("mongodb://localhost:27017/unboredEventEnv"), 
    ...
    beforeEach(async () => {
        ...
        eventUserBearer = (await request(httpServer).post('/auth/login').send(User1)).body.token
        fakeUserBearer = (await request(httpServer).post('/auth/login').send(fakeUser)).body.token
    ...
    describe('get event information', () => {
        ...
        it ('should return me an event', async() => {
            const create = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const response = await request(httpServer).get('/event/show?id='+create.body.event._id).set('Authorization', 'Bearer ' + eventUserBearer);
            expect(response.status).toBe(HttpStatus.OK);
        });

        it ('should not return me an event (bad id)', async() => {
            const badID = new Types.ObjectId();
            const response = await request(httpServer).get('/event/show?id='+badID).set('Authorization', 'Bearer ' + eventUserBearer);
    ...
    describe('creating and deleting event', () => {
        ...
        it ('should create an event', async() => {
            const response = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            expect(response.status).toBe(HttpStatus.CREATED);
        })

        it ('should return me conflict', async() => {
            await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const response = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
        ...
        it ('should delete an event', async() => {
            const create = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const response = await request(httpServer).delete('/event/deleteevent?id='+create.body.event._id).set('Authorization', 'Bearer ' + eventUserBearer);
            expect(response.status).toBe(HttpStatus.OK);
        ...
        it ('should me return an error (bad id)', async() => {
            const badID = new Types.ObjectId();
            const response = await request(httpServer).delete('/event/deleteevent?id='+badID).set('Authorization', 'Bearer ' + eventUserBearer);
            expect(response.status).toBe(HttpStatus.NOT_FOUND);
    ...
    describe('events manipulation', () => {
        ...
        it ("should edit event", async() => {
            const eventRes = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const modifiedEvent = {
            ...
            const response = await request(httpServer).put('/event/editevent?id='+eventRes.body.event._id).set('Authorization', 'Bearer ' + eventUserBearer).send({"name": "Hehe"});
            expect(response.body.event).toMatchObject(modifiedEvent);
        })
        it ("should not edit event (bad id)", async() => {
            const response = await request(httpServer).put('/event/editevent?id='+"falseid").set('Authorization', 'Bearer ' + eventUserBearer).send({"name": "Hehe"});
            expect(response.status).toBe(HttpStatus.NOT_FOUND)
        })
        it ('should not edit event (not existing id)', async() => {
            const badId = new Types.ObjectId();
            const response = await request(httpServer).put('/event/editevent?id='+badId._id).set('Authorization', 'Bearer ' + eventUserBearer).send({"name": "Hehe"});
            expect(response.status).toBe(HttpStatus.NOT_FOUND)
        ...
        it ('should not edit event (duplicate key (name)', async() => {
            await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const eventRes = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(create2EventDto);
            const response = await request(httpServer).put('/event/editevent?id='+eventRes.body.event._id).set('Authorization', 'Bearer ' + eventUserBearer).send({"name": "testevent"});
            expect(response.status).toBe(HttpStatus.CONFLICT);
    ...
    describe('add / delete event rate', () => {
        ...
        it ("should create rate in event db", async() => {
            ...
            const response = await request(httpServer).post('/event/rateevent?id='+eventId).set('Authorization', 'Bearer ' + eventUserBearer).send(rateEventDto)
        ...
        it ("should return me an error (invalid id)", async() => {
            const response = await request(httpServer).post('/event/rateevent?id='+"badId").set('Authorization', 'Bearer ' + eventUserBearer)
            expect(response.status).toBe(HttpStatus.NOT_FOUND)
        })

        it ("should return me an error (not existing id)", async() => {
            const badId = new Types.ObjectId();
            const response = await request(httpServer).post('/event/rateevent?id='+badId._id).set('Authorization', 'Bearer ' + eventUserBearer)
            expect(response.status).toBe(HttpStatus.NOT_FOUND)
        });
        ...
        it ("should delete the rate i posted", async() => {
            const eventRes = await request(httpServer).post('/event/createevent').set('Authorization', 'Bearer ' + eventUserBearer).send(createEventDto);
            const eventId = eventRes.body.event._id;
            const rateId = await request(httpServer).post('/event/rateevent?id='+eventId).set('Authorization', 'Bearer ' + eventUserBearer).send(rateEventDto)
            const response = await request(httpServer).delete('/event/removerate').set('Authorization', 'Bearer ' + eventUserBearer).send({"rateId" : rateId.body.event.rate[0].id})
            expect(response.body.rates).toMatchObject([]);
            expect(response.body.status).toBe(HttpStatus.OK);
        })

        it ("should return me an error (Not existing rate id)", async() => {
            const response = await request(httpServer).delete('/event/removerate').set('Authorization', 'Bearer ' + eventUserBearer).send({"rateId" : "nothing"})
            expect(response.status).toBe(HttpStatus.NOT_FOUND);
        })
    })
});
