...
export class ProfileService {
    ...
    async getprofilebyid(profilId: string) : Promise <{status: HttpStatus, user: User}>{
    ...
    async UpdateUser(id: string, updateUser : UpdateDto) : Promise<{status: HttpStatus, user: User}> {
        if (updateUser.role) {
    ...
    async UserActualAvatar(user : User) : Promise<{status: HttpStatus, style: Object}> {
        return {status: HttpStatus.OK, style: user.style as Object} ;
    }

    async UserAvatars(user : User) : Promise<{status: HttpStatus, unlockedStyles: Object}>{
        return {status: HttpStatus.OK, unlockedStyles: user.unlockedStyle}
    }

    async ChangeAvatar(id: string, updateAvatarDto : UpdateAvatarDto) : Promise<{status: HttpStatus, style: Object}> {
