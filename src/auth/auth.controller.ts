...
export class AuthController {
     ...
     */
    @Post('/register')
    register(@Body() registerDto: RegisterDto) : Promise<{status: HttpStatus, message: string}> {
     ...
     */
    @Post('/login')
    login(@Body() loginDto: LoginDto) : Promise<{status: HttpStatus, token : string, refresh : string}> {
    ...
    @UseGuards(JwtGuard)
    @Post('/refresh')
    refresh(@Req() req, @Headers() head) : Promise<{status: HttpStatus, token: string, refresh : string}> {
        return this.authService.refresh(req.user, head.refresh);
    }

    @Post('/login/google')
    loginGoogle(@Body() body) : Promise<{status: HttpStatus, token: string, refresh: string}> {
