...
describe('EventController', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      imports: [
        ...
        DatabaseModule.forRoot(
          `mongodb://${process.env.MONGO_USER}:${process.env.MONGO_USER_PASS}@localhost:27017/unboredEventsEnv`,
        ),
      ],
  ...
  beforeEach(async () => {
    ...
    const login1 = await request(httpServer)
      ...
      .send({ passwword: fakeUser.password });
  ...
  describe('get event information', () => {
    ...
    it('should return me an event', async () => {
      ...
      const response = await request(httpServer)
        .get('/events/show?id=' + create.body.event._id)
        .set('Authorization', 'Bearer ' + eventUserBearer);
  ...
  describe('creating and deleting event', () => {
    ...
    it('should delete an event', async () => {
      const create = await request(httpServer)
        ...
        .send(createEventDto);
      const response = await request(httpServer)
        .delete('/events/delete?id=' + create.body.event._id)
        .set('Authorization', 'Bearer ' + eventUserBearer);
      expect(response.status).toBe(HttpStatus.OK);
  ...
  describe('events manipulation', () => {
    ...
    it('should edit event', async () => {
      ...
      const response = await request(httpServer)
        .put('/events/edit?id=' + eventRes.body.event._id)
        .set('Authorization', 'Bearer ' + eventUserBearer)
        .send({ name: 'Hehe' });
