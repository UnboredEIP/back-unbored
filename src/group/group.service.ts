...
export class GroupService {
    ...
    async showGroups(user: User) : Promise<{status: HttpStatus, groups: Object[]}> {
        return {status: HttpStatus.OK, groups: user.groups}
    }

    async showGroupWithId(id: string) : Promise<{status: HttpStatus, groups: Object}> {
    ...
    async showInvitation(user: User) : Promise<{status: HttpStatus, invitations: Object[]}> {
        return {status: HttpStatus.OK, invitations: user.invitations}
    }

    async createGroup(user: User, createGroupDto : createGroupDto) : Promise<{status: HttpStatus, group: Groups}> {
    ...
    async inviteInGroup(groupId: string, userId: string) : Promise<{status: HttpStatus, message: string}> {
    ...
    async acceptInvitation(user: User, groupId: string) : Promise<{status: HttpStatus, message: string}> {
        const exists = user.invitations.some((invitations) => invitations._id === groupId)
        if (!exists) {
    ...
    async deleteInvitation(user: User, groupId: string) : Promise <{status: HttpStatus, message: string}> {
        const exists = user.invitations.some((invitations) => invitations._id === groupId)
        if (!exists)
            return {status: HttpStatus.NOT_ACCEPTABLE, message: "user did not had an invitation from this group"};
        else {
    ...
    async sendMessage(user: User, groupdId: string, sendMessageDto: sendMessageDto) : Promise<{status: HttpStatus, message: string}> {
