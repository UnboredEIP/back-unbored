...
export class ProfileController {
  ...
  @ApiTags('Profile')
  @ApiOperation({
    summary: 'Get all users, can add options to get specifics users',
  })
  ...
  async update(
    ...
    const response = await this.profileService.UpdateUser(
      req.user.id,
      updateUser,
    );
    res.status(HttpStatus.OK);
  ...
  async changeAvatar(
    ...
    const response = await this.profileService.ChangeAvatar(
      req.user.id,
      updateAvatarDto,
    );
    res.status(HttpStatus.OK);
