...
export class FriendsService {
  ...
  async showFriends(
    user: User,
  ): Promise<{ statusCode: HttpStatus; friends: Object[] }> {
  ...
  async sendInvitation(
    ...
    if (!user1) throw new NotFoundException('could not find this user');

    const exists = user1.invitations.friends.some(
      (invitations) => invitations._id.toString() === user._id.toString(),
    );
    ...
    if (exists) {
      ...
      await this.userModel.findOneAndUpdate(
        { _id: id },
        { $addToSet: { 'invitations.friends': newInvitations } },
      );
      return {
  ...
  async acceptInvitation(
    ...
    if (!exists) throw new BadRequestException('Bad request');
    await this.userModel.findOneAndUpdate(
      { _id: user.id },
      { $pull: { 'invitations.friends': invitation } },
    );
    await this.userModel.findOneAndUpdate(
      { _id: user.id },
  ...
  async deleteFriend(
    ...
    if (!exists) throw new BadRequestException('Bad requests');
    await this.userModel.findOneAndUpdate(
      { _id: user.id },
      { $pull: { friends: { _id: id } } },
    );
    await this.userModel.findOneAndUpdate(
      { _id: id },
      { $pull: { friends: { _id: user._id.toString() } } },
    );
    return {
      statusCode: HttpStatus.ACCEPTED,
