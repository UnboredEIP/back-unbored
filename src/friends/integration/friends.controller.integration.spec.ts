...
describe('FriendController', () => {
    ...
    const moduleRef = await Test.createTestingModule({
      imports: [
        ...
        DatabaseModule.forRoot(
          `mongodb://${process.env.MONGO_USER}:${process.env.MONGO_USER_PASS}@localhost:27017/unboredFriendsEnv`,
        ),
      ],
  ...
  beforeEach(async () => {
    ...
    User1Id = (
      await dbConnection.collection('users').findOne({ email: User1.email })
    )._id.toString();
    User2Id = (
      await dbConnection.collection('users').findOne({ email: User2.email })
    )._id.toString();
  }, 10000);
  ...
  describe('friends information', () => {
    it('should return me friend list', async () => {
      const response = await request(httpServer)
        .get('/friends/')
        .set('Authorization', 'Bearer ' + User1Bearer);
  ...
  describe('friends invitation managements', () => {
    it('should send an invitation to user2', async () => {
      const response = await request(httpServer)
        .post('/friends/invite?user_id=' + User2Id)
        .set('Authorization', 'Bearer ' + User1Bearer);
      const friendlist = await request(httpServer)
        .get('/friends/invitations')
        .set('Authorization', 'Bearer ' + User2Bearer);
    ...
    it('should return me an error (my id)', async () => {
      const response = await request(httpServer)
        .post('/friends/invite?user_id=' + User1Id)
        .set('Authorization', 'Bearer ' + User1Bearer);
    ...
    it('should return me an error (wrong id)', async () => {
      const response = await request(httpServer)
        .post('/friends/invite?user_id=' + 'caca')
        .set('Authorization', 'Bearer ' + User1Bearer);
      expect(response.status).toBe(HttpStatus.BAD_REQUEST);
    });
  ...
  describe('friend accept managament', () => {
    it('should accept friend invitation', async () => {
      await request(httpServer)
        ...
        .set('Authorization', 'Bearer ' + User1Bearer);
      const response = await request(httpServer)
        .post('/friends/accept?user_id=' + User1Id)
        .set('Authorization', 'Bearer ' + User2Bearer);
  ...
  describe('friend deletation management', () => {
    it('should delete friend', async () => {
      ...
      await request(httpServer)
        ...
        .set('Authorization', 'Bearer ' + User2Bearer);
      const response = await request(httpServer)
        .delete('/friends/delete?user_id=' + User1Id)
        .set('Authorization', 'Bearer ' + User2Bearer);
      expect(response.status).toBe(HttpStatus.ACCEPTED);
    });
